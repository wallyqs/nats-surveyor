...
type statzDescs struct {
	...
	// Jetstream Meta Cluster Snapshot Stats (from nats-server commit 5ed0c1498e46f93ce)
	JetstreamMetaClusterSnapshotPendingEntries *GaugeVec
	JetstreamMetaClusterSnapshotPendingBytes   *GaugeVec
	JetstreamMetaClusterSnapshotLastTime       *GaugeVec
	JetstreamMetaClusterSnapshotLastDuration   *GaugeVec
...
func (sc *StatzCollector) buildDescs() {
	...
	sc.descs.JetstreamMetaClusterSnapshotPendingEntries = newGaugeVec(newName("jetstream_meta_cluster_snapshot_pending_entries"), "Number of pending entries awaiting meta cluster snapshot", sc.constLabels, jsMetaSnapshotLabelKeys)
	sc.descs.JetstreamMetaClusterSnapshotPendingBytes = newGaugeVec(newName("jetstream_meta_cluster_snapshot_pending_bytes"), "Size in bytes of pending entries awaiting meta cluster snapshot", sc.constLabels, jsMetaSnapshotLabelKeys)
	sc.descs.JetstreamMetaClusterSnapshotLastTime = newGaugeVec(newName("jetstream_meta_cluster_snapshot_last_time"), "Unix timestamp of the last meta cluster snapshot", sc.constLabels, jsMetaSnapshotLabelKeys)
	sc.descs.JetstreamMetaClusterSnapshotLastDuration = newGaugeVec(newName("jetstream_meta_cluster_snapshot_last_duration_ns"), "Duration of the last meta cluster snapshot in nanoseconds", sc.constLabels, jsMetaSnapshotLabelKeys)
...
func (sc *StatzCollector) MetricInfos() []MetricInfo {
	metrics := []MetricInfo{
		...
		// Jetstream Meta Cluster Snapshot Stats
		sc.descs.JetstreamMetaClusterSnapshotPendingEntries,
		sc.descs.JetstreamMetaClusterSnapshotPendingBytes,
		sc.descs.JetstreamMetaClusterSnapshotLastTime,
		sc.descs.JetstreamMetaClusterSnapshotLastDuration,
	}
